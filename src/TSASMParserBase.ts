...
import { TSASMParser } from './parser/grammar/TSASMParser';

// add this to generated ./parser/grammar/TSASMParser.ts
 ...
 */
export abstract class TSASMParserBase extends Parser {
    ...
    protected notLineTerminator():boolean {
        return !this.here(TSASMParser.LineTerminator);
    }

    protected notOpenBrace():boolean {
        const nextTokenType:number = this._input.LT(1).type;
        return nextTokenType != TSASMParser.OpenBrace;
    }

    protected closeBrace():boolean {
        return this._input.LT(1).type == TSASMParser.CloseBrace;
    }
    ...
    protected lineTerminatorAhead():boolean {
        ...
        if (ahead.type == TSASMParser.LineTerminator) {
            // There is definitely a line terminator ahead.
            return true;
        }

        if (ahead.type == TSASMParser.WhiteSpaces) {
        ...
        return (type == TSASMParser.MultiLineComment && (text.includes("\r") || text.includes("\n"))) ||
                (type == TSASMParser.LineTerminator);
    }
}